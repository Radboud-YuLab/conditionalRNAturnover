## Performance of basic.codon with mean threshold applied

# only need one run, as this would be the final static performance

# run2

# changelog
## updated mtry parameter to 70 as the max
...
## identifiers
date_of_run <- "20250719" 
number_cores <- 10L
algorithm <- "all.models"
run_number <- "2"
cv_number <- 10
path_project <- "6_results/ML/w_cutoff_threshhold/static_model/"
set.seed(1998)
## take subset
take_subset = F
...
data_cv <- vfold_cv(data_train, strata = mean_hl, v = cv_number)
